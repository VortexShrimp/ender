...
namespace {
    ...
    struct FontInfo {
        ...
        float LineSpacing;  // The baseline-to-baseline distance. Note that it usually is larger
                            ...
                            // the designer of the font finds appropriate.
        float
            LineGap;  // The spacing in pixels between one row's descent and the next row's ascent.
...
#define FT_CEIL(X) (((X + 63) & -64) / 64)
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg,
        ...
        FT_Error error =
            FT_New_Memory_Face(ft_library, (uint8_t*)cfg.FontData, (uint32_t)cfg.FontDataSize,
    ...
    void FreeTypeFont::SetPixelHeight(int pixel_height) {
        // Vuhdo: I'm not sure how to deal with font sizes properly. As far as I understand,
        // currently ImGui assumes that the 'pixel_height' is a maximum height of an any given
        // glyph, i.e. it's the sum of font's ascender and descender. Seems strange to me. NB:
    ...
    const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint) {
        ...
        IM_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE ||
                  slot->format == FT_GLYPH_FORMAT_BITMAP || slot->format == FT_GLYPH_FORMAT_SVG);
#else
#if ((FREETYPE_MAJOR >= 2) && (FREETYPE_MINOR >= 12))
        IM_ASSERT(slot->format != FT_GLYPH_FORMAT_SVG &&
                  "The font contains SVG glyphs, you'll need to enable "
                  "IMGUI_ENABLE_FREETYPE_PLUTOSVG or IMGUI_ENABLE_FREETYPE_LUNASVG in imconfig.h "
                  "and install required libraries in order to use this font");
#endif
        IM_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE ||
    ...
    void FreeTypeFont::BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch,
        ...
        switch (ft_bitmap->pixel_mode) {
            ...
            case FT_PIXEL_MODE_MONO:  // Monochrome image, 1 bit per pixel. The bits in each byte
        ...
        for (const ImWchar* src_range = src_tmp.SrcRanges; src_range[0] && src_range[1];
            ...
            for (int codepoint = src_range[0]; codepoint <= (int)src_range[1]; codepoint++) {
                ...
                uint32_t glyph_index = FT_Get_Char_Index(
                    src_tmp.Font.Face, codepoint);  // It is actually in the font? (FIXME-OPT: We
